 ...
 * 
 * Usage:
 *   npx ts-node scripts/deploy-merkle-tree.ts
 * 
...
// These are balanced values for cost vs. capacity
const MAX_DEPTH = 7;
...
async function main() {
    ...
    if (balanceSOL < 0.5) {
        console.warn("⚠️  Warning: Low balance. Tree deployment may cost 0.5-2 SOL depending on size.");
        if (isDevnet) {
